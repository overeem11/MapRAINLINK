...
DrawParallelsMeridians = "yes"   		# "yes" for drawing parallels & meridians.
...
OutputFileName = "SriLanka.jpg"			# Name of output file with map. Can at least be "jpg" or "pdf".
...
LatitudesParallels = "[6, 7, 8, 9]"		# Latitudes of parallels to be plotted.
...
LongitudesMeridians = "[80, 81, 82]"		# Longitudes of meridians to be plotted.
...
LatitudeNorthArrowEnd = 8.9			# Latitude of end of north arrow.
LongitudeNorthArrowEnd = 81.6			# Longitude of end of north arrow.
LatitudeNorthArrowStart = 8.6			# Latitude of start of north arrow.
LongitudeNorthArrowStart = 81.6			# Longitude of start of north arrow.
LatitudeNorthArrowLabel = 8.9			# Latitude of north arrow label ("N").
LongitudeNorthArrowLabel = 81.65		# Longitude of north arrow label ("N").
SizeNorthArrow = 25				# Size of north arrow.				# Size of north arrow.
...
XPositionScaleBar = 0.84			# Horizontal position of scale bar. Should be value between 0-1.
YPositionScaleBar = 0.85			# Vertical position of scale bar. Should be value between 0-1.
...
FontSizeTitle = 15				# Font size of title.
TitlePlot = "Sri Lanka"			  	# Title of map.


# 3.3 Options for color bar (legend).
DistanceBetweenMapAndColorBar = 0.1		# Increase this value to make distance between map and color bar larger. Different values may be needed for different sizes of plotted areas.
						# These values could be tested as starting point: use larger value, e.g., 0.1, in case DrawParallelsMeridians = "yes", otherwise 0.02.
...
LabelName = "Rainfall intensity (mm h$^{-1}$)"	# Label name of legend. Also Latex mathematical notation is accepted, e.g.: "Rainfall intensity (mm h$^{-1}$)"
LevelsScale = "[0.3,1,2,4,8,16,32]"	# Values for legend scale. Simply add or remove values to increase or decrease the number of classes 
...
SizeLegend = 0.9				# Size of legend scale.


# 3.4 Options for extra text.
ColorExtraText = "black"			# Color of extra text.
ExtraText = "End time: 201910171045"		# Extra text.
FontSizeExtraText = 16				# Font size of extra text.
LatExtraText = 5.9				# Latitude of extra text. 
LonExtraText = 79.7				# Longitude of extra text.
...
PlotCMLTimeInterval = "no"			# "yes" for plotting link paths from file "FileNameCMLLocationsTimeInterval", i.e. those from the specific time interval. 
...
FileNameCMLLocationsTimeInterval  = "linkdata_201909231900.dat" 	# File name for file with commercial microwave link locations for specific time interval.
...
# 5.2 Other options.
ConversionFactorCMLDataField = 1		# Conversion factor in case rainfall accumulations or intensities need to be converted. For interpolated rainfall data.
						# E.g. use 4 to convert 15-min rainfall accumulations to intensities in millimeters per hour.
...
InterpolationGrid = "CMLInterpolationGridSriLanka.dat"	# Filename of file with grid coordinates.
...
InputFileNameDataField = "linkmap_201910171615_SriLanka.dat"	# Filename of file with interpolated rainfall map data. Time zone is UTC+5:30, Indian Standard Time, which is the local time in Sri Lanka.
# "linkmap_201910171615_SriLanka.dat" is file "linkmap_201910171615.dat" from https://doi.org/10.4121/14166539.v2 
# Here, the CML rainfall map from Figure 5(e) from https://doi.org/10.1088/1748-9326/ac0fa6 is reproduced.
...
FileNameGaugeLocationsNetwork1 = "StationMetadataDailyMetDepartmentTest.dat"	# Filename with gauge locations for network 1.
FileNameGaugeLocationsNetwork2 = "StationMetadataHourlyMetDepartmentTest.dat"	# Filename with gauge locations for network 2.
FileNameGaugeLocationsNetwork3 = "StationMetadataHourlyMetDepartmentTest.dat"	# Filename with gauge locations for network 3.



# 6.3 Settings for markers.
ColorGaugeNetwork1 = "darkblue"			# Color of marker for gauge network 1.
...
MarkerGaugeNetwork1 = "D"			# Symbol for marker of gauge network 1.
...
FontSizeLabelNameGaugesNetwork1 = 9		# Font size of label name for gauge network 1.
FontSizeLabelNameGaugesNetwork2 = 9		# Font size of label name for gauge network 2.
FontSizeLabelNameGaugesNetwork3 = 9		# Font size of label name for gauge network 3.
FontSizeNamesGaugesNetwork1 = 9			# Font size of (abbreviated) names/IDs for gauge network 1.
FontSizeNamesGaugesNetwork2 = 9			# Font size of (abbreviated) names/IDs for gauge network 2.
FontSizeNamesGaugesNetwork3 = 9			# Font size of (abbreviated) names/IDs for gauge network 3.
LabelNameGaugesNetwork1 = "DM, 24 h"		# Label name for gauge network 1.
LabelNameGaugesNetwork2 = "DM, 1 h"		# Label name for gauge network 2.
LabelNameGaugesNetwork3 = "Network 3"		# Label name for gauge network 3.
LatitudeLabelGaugesNetwork1 = 9.5		# Longitude of label for gauge network 1 (degrees).
LatitudeLabelGaugesNetwork2 = 9.3		# Longitude of label for gauge network 2 (degrees).
LatitudeLabelGaugesNetwork3 = 9.1		# Longitude of label for gauge network 3 (degrees).
LatitudeOffsetLabelNameAndNamesGaugesNetwork1 = 0.08	# Offset for longitude for plotting of label name and names of gauges for network 1.
LatitudeOffsetLabelNameAndNamesGaugesNetwork2 = 0.08	# Offset for longitude for plotting of label name and names of gauges for network 2.
LatitudeOffsetLabelNameAndNamesGaugesNetwork3 = 0.08	# Offset for longitude for plotting of label name and names of gauges for network 3.
LongitudeLabelGaugesNetwork1 = 81.06		# Latitude of label for gauge network 1 (degrees).
LongitudeLabelGaugesNetwork2 = 81.06		# Latitude of label for gauge network 2 (degrees).
LongitudeLabelGaugesNetwork3 = 81.06		# Latitude of label for gauge network 3 (degrees).
LongitudeOffsetLabelNameAndNamesGaugesNetwork1 = -0.035	# Offset for latitude for plotting of label name and names of gauges for network 1.
LongitudeOffsetLabelNameAndNamesGaugesNetwork2 = -0.035	# Offset for latitude for plotting of label name and names of gauges for network 2.
LongitudeOffsetLabelNameAndNamesGaugesNetwork3 = -0.035	# Offset for latitude for plotting of label name and names of gauges for network 3.
NumberOfCharactersNamesNetwork1 = 4            	# Number of characters of names/IDs of rain gauge locations to plot for network 1. Set to a large value to plot entire names/IDs. 
...
KNMIRadarInputFileName = "RAD_NL25_RAC_MFBS_24H_201805300800_NL.h5"	# Filename of file with interpolated rainfall map data.
...
OPERARadarInputFileName = "RAD_OPERA_24H_RAINFALL_ACCUMULATION_201305311400.h5"	# Filename of file with interpolated rainfall map data.
